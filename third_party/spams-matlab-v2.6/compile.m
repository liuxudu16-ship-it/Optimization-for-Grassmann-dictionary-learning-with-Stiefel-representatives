...
use_multithread=false; % (avoid OpenMP on macOS clang)
...
% On Apple Silicon, use the C mexopts file to avoid C++ linker issues.
mex_opts_file = '';
if strcmp(compiler,'mex') && mac
    mex_opts_file = fullfile(matlabroot, 'bin', 'maca64', 'mexopts', 'clang_maca64.xml');
end
...
elseif strcmp(blas,'builtin')
    ...
    blas_link = ['-L"' libdir '" -lmwblas -lmwlapack'];
    DEFBLAS='-DUSE_BLAS_LIB';
elseif strcmp(blas,'acml')
...
for k = 1:length(COMPILE),
    ...
    if ~isempty(mex_opts_file) && exist(mex_opts_file, 'file')
        str = ['-f ' mex_opts_file ' ' str];
    end
